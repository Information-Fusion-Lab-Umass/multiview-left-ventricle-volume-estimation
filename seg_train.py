...
def parse_args():
    ...
    parser.add_argument('--axis', default='SA',
                        choices=['SA', 'LA'],
                        help='axis: ' +
                        ' | '.join(['SA', 'LA']) +
                        ' (default: SA)')
...
def main():
    ...
    # Get patient ids and divide into train and val sets
    # f = open('ids.txt', 'r')
    # Val set contains 20/100 patients
    #lines = f.readlines()
    val_idx = []
    with open('ids_group1.txt', 'r') as file: # Other ids groups: ids_group2, ids_group3, ids_group4, ids_group5
        val_idx = [line.rstrip() for line in file]
    ...
    for epoch in range(config['epochs']):
        ...
        # can also use iou score instead of dice score for saving the best model
